...
<form method="POST" action="oracle-test-mod.php">
...
<!--refresh page when submit-->
   ...
   <p><input type="text" name="insNo" size="6"><input type="text" name="insName" size="18">
   <input type="text" name="insFav_col" size="18">
...
function printResult($result) { //prints results from a select statement
	...
	while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
		echo "<tr><td>" . $row["NID"] . "</td><td>" . $row["NAME"] . "</td><td>" . $row["NAME"]."</td></tr>"; //or just use "echo $row[0]" 
	}
	echo "</table>";
...
if ($db_conn) {
	...
	if (array_key_exists('reset', $_POST)) {
		...
		executePlainSQL("create table tab1 (nid number, name varchar2(30), fav_color varchar2(10), primary key (nid))");
		OCICommit($db_conn);
		...
		if (array_key_exists('insertsubmit', $_POST)) {
			...
			if (array_key_exists('updatesubmit', $_POST)) {
				...
				if (array_key_exists('dostuff', $_POST)) {
					...
					executePlainSQL("insert into tab1 values (10, 'Frank', 'red')");
	...
	if ($_POST && $success) {
		...
		header("location: oracle-test-mod.php");
	} else {
		// Select data...
